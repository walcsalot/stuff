<?php
  ...
  $connect->link('user_login_page.php?AAAAAAAAAAAaaaa');
  }
  ...
  /*if(isset($_POST['btn-tasks'])){
    $task = $_POST['tasks'];
    $connect->insertTask($task,$user_id);
  }
  
  $alltask = $dbconn->query("SELECT * FROM tasks WHERE user_id = $user_id ORDER BY id DESC"); */
